...
- [x] **Intermediate commits during pipeline run** — Refactored to per-job commits. Each job is committed immediately after each state change (extracted, pre-filtered, scored, tailored, applied). No more batch transaction. Jobs appear in web app in real-time, no 504s, no rollback cascade.
...
- [ ] **Track all discovered form fields in application_notes** — When the Vision Agent extracts DOM fields during an apply attempt, store ALL fields (filled and unfilled) in the job's `application_notes`. Include: field label, whether it was filled, what value was used (or "unfilled" / "file_upload" / "skipped_optional"). This gives visibility into what the form looked like and what was missed. If required fields are left unfilled, escalate to human queue instead of submitting a partial application.
- [ ] **Escalate on low fill rate** — If fewer than 50% of non-optional, non-file fields are filled, don't submit. Instead mark as `apply_failed` with reason "too many unfilled fields" and notify the user. Better to not submit than to submit garbage.
