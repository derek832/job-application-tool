...
- [x] 1. Data layer: new models, schema migration, and config keys
  - [x] 1.1 Add `RunSummary` ORM model and `channel` column to `NotificationLog`
  ...
  - [x] 1.2 Create ntfy config key seeding in startup
  ...
  - [x] 1.3 Write property tests for topic generation
    - **Property 2: Topic Generation Produces Valid Hex Strings**
    - **Property 3: Topic Initialization Idempotence**
    - **Validates: Requirements 2.1, 2.3**

- [x] 2. Ntfy client and topic generator
  - [x] 2.1 Implement `src/integrations/ntfy_client.py`
  ...
  - [x] 2.2 Implement `src/integrations/ntfy_topic_gen.py`
  ...
  - [x] 2.3 Write property tests for ntfy payload composition
    - **Property 1: Urgent Notification Payload Completeness**
    - **Validates: Requirements 1.2, 1.5**

  - [x] 2.4 Write property tests for action button conditional inclusion
    - **Property 4: Action Button Conditional Inclusion**
    - **Property 5: Action Button URL Construction**
    - **Validates: Requirements 3.1, 3.2, 3.3, 3.6**

- [x] 3. Notification service refactor
  - [x] 3.1 Refactor `src/pipeline/notification_service.py` into channel router
  ...
  - [x] 3.2 Implement ntfy message composition helpers
  ...
  - [x] 3.3 Update rate limiter to be channel-agnostic
  ...
  - [x] 3.4 Write property tests for notification channel routing
    - **Property 8: Notification Channel Routing**
    - **Validates: Requirements 8.1, 8.2, 8.3, 8.5**

  - [x] 3.5 Write property tests for rate limit enforcement
...
- [x] 5. Run summary generator and API
  - [x] 5.1 Implement `src/pipeline/run_summary.py`
  ...
  - [x] 5.2 Create `src/api/run_routes.py` with `GET /runs/history` endpoint
  ...
  - [x] 5.3 Write property tests for run summary generation and retention
    - **Property 6: Run Summary Generation Correctness**
    - **Property 7: Run Summary Retention Policy**
    - **Validates: Requirements 5.1, 5.2, 5.5**

  - [x] 5.4 Write property test for run history pagination
    - **Property 11: Run History Pagination**
    - **Validates: Requirements 10.1, 10.2, 10.4**

- [x] 6. LAN server binding
  - [x] 6.1 Implement `src/api/lan_server.py`
  ...
  - [x] 6.2 Integrate LAN server startup into application lifespan
  ...
  - [x] 6.3 Write unit tests for LAN server creation
    - Verify only queue + health routes are mounted
    - Verify auth is required on all LAN endpoints
    - _Requirements: 4.2, 4.3_

- [x] 7. Ntfy configuration API endpoints
  - [x] 7.1 Add `GET /config/ntfy` and `PUT /config/ntfy` endpoints
...
- [x] 9. Web App: Ntfy settings UI
  - [x] 9.1 Add ntfy settings section to `webapp/src/pages/Settings.tsx`
  ...
  - [x] 9.2 Add API client methods for ntfy config
...
- [x] 10. Web App: Run History dashboard section
  - [x] 10.1 Add Run History section to `webapp/src/pages/Dashboard.tsx`
  ...
  - [x] 10.2 Add API client method for run history
    - Add `getRunHistory(limit?)` to `webapp/src/api/client.ts`
    - Add Zod schema for run history response validation
    - _Requirements: 10.1, 10.2_

- [x] 11. Integration wiring: pipeline → notification service
  - [x] 11.1 Wire pipeline stages to use refactored notification service
  ...
  - [x] 11.2 Write integration tests for end-to-end notification flow
