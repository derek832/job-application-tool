...
- [x] 1. Database models and migrations
  - [x] 1.1 Add SQLAlchemy models for preview_runs, preview_jobs, blacklist_entries, and notification_queue tables
  ...
  - [x] 1.2 Create Alembic migration or table-creation logic for the new tables
...
- [x] 2. Blacklist filter module
  - [x] 2.1 Implement `automator/src/pipeline/blacklist_filter.py` with `BlacklistConfig` dataclass and `check_blacklist()` function
  ...
  - [x] 2.2 Write property test for blacklist matching correctness
    - **Property 13: Blacklist Matching Correctness**
    - **Validates: Requirements 4.5, 4.11**

  - [x] 2.3 Implement blacklist database repository in `automator/src/db/blacklist_repo.py`
  ...
  - [x] 2.4 Write unit tests for blacklist repository CRUD operations
...
- [x] 4. Session health checker
  - [x] 4.1 Implement `automator/src/pipeline/health_checker.py` with `HealthCheckResult` dataclass and `check_session_health()` function
  ...
  - [x] 4.2 Write property test for health check failure notification specificity
    - **Property 5: Health Check Failure Notification Specificity**
    - **Validates: Requirements 2.4, 2.9**

  - [x] 4.3 Integrate health check into pipeline entry point
...
- [x] 5. Chrome CDP launcher
  - [x] 5.1 Implement `automator/src/integrations/chrome_launcher.py` with `get_chrome_status()` and `launch_chrome()` functions
  ...
  - [x] 5.2 Write property test for Chrome launch command correctness
    - **Property 14: Chrome Launch Command Correctness**
    - **Validates: Requirements 5.3, 5.4, 5.8**

- [x] 6. Preview pipeline mode
  - [x] 6.1 Implement `automator/src/pipeline/preview_pipeline.py` with `run_preview()` function
  ...
  - [x] 6.2 Write property test for preview mode never advancing beyond scoring
    - **Property 1: Preview Mode Never Advances Beyond Scoring**
    - **Validates: Requirements 1.1**

  - [x] 6.3 Write property test for preview result persistence completeness
    - **Property 2: Preview Result Persistence Completeness**
    - **Validates: Requirements 1.2**

  - [x] 6.4 Write property test for preview deduplication
    - **Property 4: Preview Deduplication**
    - **Validates: Requirements 1.9**

  - [x] 6.5 Implement preview job promotion logic
  ...
  - [x] 6.6 Write property test for preview job promotion state transition
...
- [x] 8. Schedule manager
  - [x] 8.1 Implement `automator/src/scheduler/schedule_manager.py` with `ScheduleConfig` dataclass, `compute_next_run_times()`, and `apply_schedule()` functions
  ...
  - [x] 8.2 Write property test for specific times schedule correctness
    - **Property 6: Specific Times Schedule Correctness**
    - **Validates: Requirements 3.1**

  - [x] 8.3 Write property test for interval schedule correctness
    - **Property 7: Interval Schedule Correctness**
    - **Validates: Requirements 3.2**

  - [x] 8.4 Write property test for weekend day filtering
    - **Property 8: Weekend Day Filtering**
    - **Validates: Requirements 3.4, 3.5, 3.6**

  - [x] 8.5 Write property test for schedule validation rejecting zero times
    - **Property 11: Schedule Validation Rejects Zero Times**
    - **Validates: Requirements 3.12**

- [x] 9. Quiet hours manager
  - [x] 9.1 Implement `automator/src/pipeline/quiet_hours.py` with `is_quiet_hours()`, `queue_notification()`, and `flush_notification_queue()` functions
  ...
  - [x] 9.2 Write property test for quiet hours notification queueing
    - **Property 9: Quiet Hours Notification Queueing**
    - **Validates: Requirements 3.8**

  - [x] 9.3 Write property test for quiet hours batch delivery
    - **Property 10: Quiet Hours Batch Delivery**
    - **Validates: Requirements 3.9**

  - [x] 9.4 Integrate quiet hours into notification service
...
- [x] 11. Backend API endpoints
  - [x] 11.1 Create `automator/src/api/preview_routes.py` with preview endpoints
  ...
  - [x] 11.2 Create `automator/src/api/health_routes.py` with session health endpoint
    - `GET /health/session`: perform session health check, return structured result
    - Enforce 15-second timeout
    - _Requirements: 2.5, 2.6_

  - [x] 11.3 Add schedule endpoints to `automator/src/api/config_routes.py`
  ...
  - [x] 11.4 Add blacklist endpoints to `automator/src/api/config_routes.py`
  ...
  - [x] 11.5 Write property test for blacklist configuration round-trip
    - **Property 12: Blacklist Configuration Round-Trip**
    - **Validates: Requirements 4.1, 4.2, 4.10**

  - [x] 11.6 Create `automator/src/api/chrome_routes.py` with Chrome CDP endpoints
  ...
  - [x] 11.7 Register all new route modules in `automator/src/main.py`
...
- [x] 13. Frontend — Preview mode UI
  - [x] 13.1 Create `webapp/src/pages/PreviewResults.tsx` page component
  ...
  - [x] 13.2 Add "Preview Run" button to Dashboard and preview status indicator
...
- [x] 14. Frontend — Session health and Chrome status UI
  - [x] 14.1 Add session health indicators and Chrome status to Dashboard
...
- [x] 15. Frontend — Schedule configuration UI
  - [x] 15.1 Create schedule configuration section in Settings page
...
- [x] 16. Frontend — Blacklist configuration UI
  - [x] 16.1 Create `webapp/src/pages/BlacklistConfig.tsx` page component
...
- [x] 17. Frontend routing and navigation
  - [x] 17.1 Wire new pages into App router and navigation
...
- [x] 18. Integration wiring and pipeline integration
  - [x] 18.1 Integrate blacklist filter into the full pipeline run
  ...
  - [x] 18.2 Wire schedule manager into application startup
